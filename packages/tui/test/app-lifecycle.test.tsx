...
test.each([false, true])(
  "keeps assistant footer metrics current across history repair and revert (older rows removed: %s)",
  async (removeOlder) => {
    await using state = await tmpdir()
    const session = {
      id: "ses_footer",
    ...
    await using setup = await createAppFixture({
      ...
      fetch: (url) => {
        if (url.pathname === `/api/session/${session.id}`) return json({ data: session })
        if (url.pathname === `/api/session/${session.id}/message`) {
          if (url.searchParams.get("cursor") === "older" && refresh && removeOlder)
            return json({ data: [], cursor: {} })
          ...
          return json({
            ...
            cursor: refresh && removeOlder ? {} : { next: "older" },
          })
        }
        if (
          url.pathname === `/api/session/${session.id}/inbox` ||
    ...
    const initial = await setup.waitForFrame(
      (frame) => frame.includes("Original answer") && frame.includes("20.0 tok/s"),
    )
    ...
    // Two new rows arrive offline. When canonical retention removes the older
    // pair, length stays four and the retained assistant moves from index three
    // to one. Otherwise all six rows must remain through reconnect repair.
    refresh = true
    ...
    const refreshed = await setup.waitForFrame(
      (frame) =>
        ...
        frame.includes("Prepended input") === !removeOlder &&
        frame.includes("Prepended instructions") === !removeOlder,
      { maxPasses: 120 },
    )
